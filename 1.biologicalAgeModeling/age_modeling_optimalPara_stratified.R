...
organs_data <- c('brain_clean.csv', 'Brain_WM_clean.csv', 'Brain_GM_clean.csv','heart_clean.csv', 'kidney_clean.csv', 'liver_clean.csv','Pancreas_clean.csv', 'OCT_clean.csv', 'sub_clean.csv','bone_clean.csv','boneSub_clean.csv','body_clean.csv','overall_clean.csv')
genders <- c(0,1)
genderNames <- c('Female','Male')
sink('/data/ageModeling/stratified/imagingvisit_ageModeling_log.txt')
########################## model traning and testing on normal controls
for(gender in genders){
  for (i in 1:length(organs)) {
    print(paste(organs[i],gender,sep=':'))
    organ <- organs[i]
    ...
    if (organ != 'OCT') {
      ...
      age <- age[,c('eid','age2','Sex')]
      df <- inner_join(df, age, by = 'eid')
      df <- rename(df, Age = age2)
    } else {
      age <- read.csv(cova_data_path)
      age <- age[,c('eid','Age','Sex')]
      df <- inner_join(df, age, by = 'eid')
    }
    if(organ=='bone' | organ=='boneSub' | organ=='body' | organ=='overall'){
      df$"X20319.2.0"=NULL
    }
    ...
    df_HC <- data.frame(df)
    write_csv(df_HC, paste0('/data/ageModeling/stratified/', organ, '_age_prediction_',genderNames[gender+1],'.csv'))
    save.image(file = paste('/data/ageModeling/stratified/',organ,"_AgeModeling_environment_",genderNames[gender+1],".RData",sep=''))
    ...
    write.table(corrdata, paste0('/data/ageModeling/stratified/', organ, '_age_prediction_evaluation_',genderNames[gender+1],'.csv'), sep=',', row.names=F)
    ...
    write_csv(brain_traits, paste0('/data/ageModeling/stratified/', organ, 'Age_LASSO_weights_',genderNames[gender+1],'.csv'))
    ...
    df[,2:ncol(df)] <- predict(best_scaler, df[,2:ncol(df)])
    ...
    write_csv(data.frame(df_all), paste0('/data/ageModeling/stratified/', organ, '_age_prediction_all_',genderNames[gender+1],'.csv'))
...
for(gender in genders){
  for(organ in organs){
    lasso_weights <- read.table(paste0('/data/ageModeling/stratified/', organ, 'Age_LASSO_weights_',genderNames[gender+1],'.csv'),sep=',',header=T)
    output <- merge(lasso_weights, fieldnames,by='field',all.x=T)
    write.table(output, paste0('/data/ageModeling/stratified/', organ, 'Age_LASSO_weights_',genderNames[gender+1],'_annotated.csv'), sep=',', row.names=F)
  }
}
