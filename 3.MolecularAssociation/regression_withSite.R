...
organ <- organs[i]
# relationship between protein and age gap
X <- read.table(paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'Age_original_INT_AgeSexremoved_protemic.csv'),sep = ',',header=FALSE)
colnames(X)[colnames(X)=='V1'] <- 'eid'
X_orig <- as.matrix(X[,2:dim(X)[2]])
Y <- read.table(paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'Age_original.csv'),sep = ',',header=TRUE)
Y_orig <- as.matrix(Y[,2:dim(Y)[2]])
# check if the subjs in same order
X_order <-  read.table(paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'Age_original_subjs.csv'),sep = ',',header=TRUE)
Y_order <- read.table(paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'Age_original_subjs.csv'),sep = ',',header=TRUE)
...
if(organ=='OCT'){
cov_orig <- cov_orig[,c('eid','age0','Sex','eTIV','Education','Townsend_index','BMI0','smoking0','drinking0','Ethnicity')]
cov_orig <- cov_orig[!apply(cov_orig,1,function(x){any(is.nan(x))}),]
cov_orig <- cov_orig[!apply(cov_orig,1,function(x){any(is.na(x))}),]
cov_orig <- merge(cov_orig, allSiteInfo[,c('eid','54-0.0')], by='eid')
colnames(cov_orig)[colnames(cov_orig)=='54-0.0'] <- 'Site'
cov_orig <- fastDummies::dummy_cols(cov_orig, select_columns = "Site", remove_first_dummy = TRUE)
...
siteCol <- colnames(cov_orig)[grepl('Site_',colnames(cov_orig))]
...
cov_orig <- cov_orig[,c('eid','age0','Sex','Education','Townsend_index','BMI0',siteCol, smokingCol,drinkingCol,ethnicityCol)]
}else{
cov_orig <- cov_orig[,c('eid','age2','interval','Sex','eTIV','Education','Townsend_index','BMI2','smoking2','drinking2','Ethnicity')]
cov_orig <- cov_orig[!apply(cov_orig,1,function(x){any(is.nan(x))}),]
cov_orig <- cov_orig[!apply(cov_orig,1,function(x){any(is.na(x))}),]
cov_orig <- merge(cov_orig, allSiteInfo[,c('eid','54-2.0')], by='eid')
colnames(cov_orig)[colnames(cov_orig)=='54-2.0'] <- 'Site'
cov_orig <- fastDummies::dummy_cols(cov_orig, select_columns = "Site", remove_first_dummy = TRUE)
...
ethnicityCol <- colnames(cov_orig)[grepl('Ethnicity_',colnames(cov_orig))]
cov_orig <- cov_orig[,c('eid','age2','interval','Sex','Education','Townsend_index','BMI2',siteCol,smokingCol,drinkingCol,ethnicityCol)]
}
ageGapNames=organ
proteinNames=read.table('/data/imputated_median_scaled_bl_proteinNames.csv', sep = ',', header=TRUE)
...
write.table(tempout,paste0('/data/ProteomicAsso/association_final_',organ,'Data.csv'),row.names = F,sep=',')
demography_subjs <- rownames(C)
if(organ=='OCT'){
  demography_ageGap <- cov[demography_subjs,c('eid','age0','Sex','eTIV','Education','Townsend_index','BMI0','smoking0','drinking0','Ethnicity')]
}else{
  demography_ageGap <- cov[demography_subjs,c('eid','age2','interval','Sex','eTIV','Education','Townsend_index','BMI2','smoking2','drinking2','Ethnicity')]
}
demography_ageGap$Group <- rep(organ,nrow(demography_ageGap))
saveRDS(demography_ageGap,paste0('/data/ProteomicAsso/demography_',organ,'.rds'))


## model linear regression between multiple proteins and age gap
...
perform_regression <- function(i, j, modeltype='gaussian',modality) {
  ...
  if(modeltype=='gaussian'){
    ...
    if (modality=='OCT'){
      model <- glm(as.formula(paste0('y~x+age0+Sex+Education+Townsend_index+',paste(siteCol,sep='+'),'+',paste(smokingCol,sep='+'),'+',paste(drinkingCol,sep='+'),'+',paste(ethnicityCol,sep='+'))), cbind(data.frame(x,y),C),family=gaussian)
    }else{
      model <- glm(as.formula(paste0('y~x+age2+interval+Sex+Education+Townsend_index+',paste(siteCol,sep='+'),'+',paste(smokingCol,sep='+'),'+',paste(drinkingCol,sep='+'),'+',paste(ethnicityCol,sep='+'))), cbind(data.frame(x,y),C),family=gaussian)
    }
  }
...
statistics_result <- function(pmatrix, dim_index, threshold=0.05){
  ...
  if(dim_index==1){
    ...
    # get sig agegaps for each protein
...
agegaps_sig_oneProtein_overall_BH <- statistics_result(padj_BH_overall, 1)[[4]]
agegaps_sig_oneProtein_overall_BH_beta <- statistics_result(padj_BH_overall, 1)[[5]]
agegaps_sig_oneProtein_overall_BH_se <- statistics_result(padj_BH_overall, 1)[[6]]
agegaps_sig_oneProtein_overall_BH_pvalue <- statistics_result(padj_BH_overall, 1)[[7]]
agegaps_sig_oneProtein_overall_BH_adjpvalue <- statistics_result(padj_BH_overall, 1)[[8]]
...
proteins_sig_oneageGap_overall_BH <- statistics_result(padj_BH_overall, 2)[[4]]
proteins_sig_oneageGap_overall_BH_beta <- statistics_result(padj_BH_overall, 2)[[5]]
proteins_sig_oneageGap_overall_BH_se <- statistics_result(padj_BH_overall, 2)[[6]]
proteins_sig_oneageGap_overall_BH_pvalue <- statistics_result(padj_BH_overall, 2)[[7]]
proteins_sig_oneageGap_overall_BH_adjpvalue <- statistics_result(padj_BH_overall, 2)[[8]]
...
my_data_list <- list(sig_oneProtein_overall_BH, sig_oneProtein_overall_BH_all, sig_oneageGap_overall_BH,sig_oneageGap_overall_BH_all, agegaps_sig_oneProtein_overall_BH,agegaps_sig_oneProtein_overall_BH_beta,agegaps_sig_oneProtein_overall_BH_se,agegaps_sig_oneProtein_overall_BH_pvalue,agegaps_sig_oneProtein_overall_BH_adjpvalue,proteins_sig_oneageGap_overall_BH,proteins_sig_oneageGap_overall_BH_beta,proteins_sig_oneageGap_overall_BH_se,proteins_sig_oneageGap_overall_BH_pvalue,proteins_sig_oneageGap_overall_BH_adjpvalue,ageGapNames,proteinNames,sig_oneProtein_overall_BH_category, results_beta, results_se, results_tvalue,results_pvalue, padj_BH_overall)
names(my_data_list) <- c('sig_oneProtein_overall_BH','sig_oneProtein_overall_BH_all','sig_oneageGap_overall_BH','sig_oneageGap_overall_BH_all','agegaps_sig_oneProtein_overall_BH','agegaps_sig_oneProtein_overall_BH_beta','agegaps_sig_oneProtein_overall_BH_se','agegaps_sig_oneProtein_overall_BH_pvalue','agegaps_sig_oneProtein_overall_BH_adjpvalue','proteins_sig_oneageGap_overall_BH','proteins_sig_oneageGap_overall_BH_beta','proteins_sig_oneageGap_overall_BH_se','proteins_sig_oneageGap_overall_BH_pvalue','proteins_sig_oneageGap_overall_BH_adjpvalue','ageGapNames','proteinNames','sig_oneProtein_overall_BH_category','results_beta','results_se','results_tvalue','results_pvalue','padj_BH_overall')
saveRDS(my_data_list, file = paste('/public/home/pren/MultiOrganAging/data/ProteomicAsso/',prefix,"_result.rds",sep=''))
save.image(file = paste('/public/home/pren/MultiOrganAging/data/ProteomicAsso/',prefix,"_environment.RData",sep=''))
}

# actually used to get p of all proteins
organs <- c('Brain_GM','Brain_WM','heart','boneSub','liver','kidney','Pancreas')
for(i in seq(1,length(organs))){
  organ <- organs[i]
  result <- readRDS(paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'_allremoved_result.rds'))
  temp <- data.frame(result$proteinNames, result$results_beta, result$results_pvalue, result$padj_BH_overall)
  colnames(temp) <- c('proteinName', 'Beta', 'pvalue', 'adjP') 
  write.table(temp, paste0('/data/ProteomicAsso/imputated_median_scaled_bl_intersection_',organ,'_allremoved_result_sigResultBeta.csv'),sep=',',row.names = F)
}