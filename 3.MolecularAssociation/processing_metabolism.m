%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% get intersection of metabolic and age gap data
preprocessingMethod = 'imputated_median_scaled';
% imputated, imputated_scaled, noImputeScale

organs={'Brain_WM','Brain_GM','heart','kidney','liver','boneSub','OCT','Pancreas'};
for organ_index=1:length(organs)
organ = organs{organ_index};
% read prepocessed metabolism by wenqing 
dataArray = readtable(['/data/MetabolicAsso/imputated_Metabolicdata_bl.csv'],'Delimiter',',');
...
writetable(table(intersection_subjs),['/data/MetabolicAsso/' preprocessingMethod '_bl_metabolism_intersection_',organ,'Age_original_subjs.csv'],'Delimiter',',');
writetable(intersection_ageGap,['/data/MetabolicAsso/' preprocessingMethod '_bl_metabolism_intersection_',organ,'Age_original.csv'],'Delimiter',',');
...
writetable(array2table([eid X_corrected]),['/data/MetabolicAsso/' preprocessingMethod '_bl_intersection_',organ,'Age_original_INT_AgeSexremoved_metabolism.csv'],'Delimiter',',','WriteVariableNames',false);
end