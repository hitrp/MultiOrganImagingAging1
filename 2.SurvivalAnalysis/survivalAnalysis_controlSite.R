...
agingfiles <- sapply(organs,function(x) paste0(x,'_age_prediction_all_residual.csv'))
...
for(i in seq(1,length(agingfiles))){
  ...
  for(j in seq(1,length(targetfiles))){
    ...
    targetdata <- read.csv(paste0('/data/Targets_RAW/',targetfile),sep=',',header=T)
    ...
    # survival analysis
    if(organ!='OCT'){
      cleaned_data <- useddata[,c('Age','delta_corrected_corrected','Sex','target_y','time','smoking2_1','smoking2_2','drinking2_1','drinking2_2','BMI2','Education')]
      colnames(cleaned_data) <- c('Age','delta_corrected_corrected','Sex','status','years','smoking2_1','smoking2_2','drinking2_1','drinking2_2','BMI2','Education')
      cleaned_data$delta_corrected_corrected <- scale(cleaned_data$delta_corrected_corrected)
      cox_fit <- coxph(Surv(years, status) ~ delta_corrected_corrected+Sex+Age+smoking2_1+smoking2_2+drinking2_1+drinking2_2+BMI2+Education, data=cleaned_data)
    }else{
      cleaned_data <- useddata[,c('Age','delta_corrected_corrected','Sex','target_y','time','smoking0_1','smoking0_2','drinking0_1','drinking0_2','BMI0','Education')]
      colnames(cleaned_data) <- c('Age','delta_corrected_corrected','Sex','status','years','smoking0_1','smoking0_2','drinking0_1','drinking0_2','BMI0','Education')
      cleaned_data$delta_corrected_corrected <- scale(cleaned_data$delta_corrected_corrected)
      cox_fit <- coxph(Surv(years, status) ~ delta_corrected_corrected+Sex+Age+smoking0_1+smoking0_2+drinking0_1+drinking0_2+BMI0+Education, data=cleaned_data)
    }
...
write.table(resultframe, '/data/Survival/allSubjs_cox_moreCova_residual.csv',sep=',',row.names = F)