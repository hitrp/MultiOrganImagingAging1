...
for tgt_name in tqdm(tgt_name_lst):
    ...
    try:
        age_index = mydf.columns.get_loc(Age)
        omics_f_lst = mydf.columns.tolist()[3:age_index]
        ...
        y_pred_train.to_csv(f'/data/{trial}/RiskScore_feature/' + tgt_name + '.csv',index=False)
    except:
        bad_tgt_lst.append(tgt_name)

bad_tgt_df = pd.DataFrame(bad_tgt_lst)
bad_tgt_df.to_csv(f'/data/{trial}/Bad_RS_f_Target.csv', index=False)